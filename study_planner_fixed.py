    ...
    with container:
        ...
        with ui.column().classes('w-full gap-4'):
            ...
            with ui.card().classes('w-full p-4'):
                ...
                with ui.tab_panels(tabs, value='Day View').classes('w-full mt-2') as tab_panels:
                    with ui.tab_panel('Day View') as day_panel:
                        show_day_view()
            ...
            with ui.card().classes('w-full p-4 mt-4'):
                with ui.row().classes('w-full justify-between items-center'):
                    ...
                    ui.button('Add Task', on_click=lambda: show_add_task_dialog(tab_panels, day_panel, task_container)).classes('bg-green-600 text-white text-xs')
                
                # Create a container for tasks that we can clear and reload
                task_container = ui.column().classes('w-full gap-2 mt-2')
                load_tasks(tab_panels, day_panel, task_container)
...
def load_tasks(tab_panels=None, day_panel=None, task_container=None):
    ...
    try:
        ...
        # Clear existing tasks if container is provided
        if task_container:
            task_container.clear()
            
        # Create new task rows in the provided container or a new one
        with task_container or ui.column().classes('w-full gap-2 mt-2'):
            for task in tasks:
                ...
                with ui.row().classes('w-full items-center p-2 hover:bg-gray-100 rounded'):
                    ui.checkbox(value=task['status'] == 'completed', 
                              on_change=lambda e, task_id=task_id: complete_task(task_id, e.value, tab_panels, day_panel, task_container)).classes('mr-2')
                    with ui.column().classes('flex-grow'):
                    ...
                    ui.button(icon='delete', on_click=lambda task_id=task_id: delete_task(task_id, tab_panels, day_panel, task_container)).classes('text-red-600')
    except Exception as e:
        ui.notify(f"Error loading tasks: {str(e)}", color='negative')
        print(f"Database error: {str(e)}")  # For debugging

def show_add_task_dialog(tab_panels=None, day_panel=None, task_container=None):
...
def save_task(title, due_date, due_time, priority, description, dialog, tab_panels=None, day_panel=None, task_container=None):
    ...
    try:
        ...
        # Refresh the task list and calendar views
        load_tasks(tab_panels, day_panel, task_container)
        if tab_panels:
            tab_panels.set_value('Day View')
            # Clear and rebuild the day view
            if day_panel:
                day_panel.clear()
                show_day_view()
            ui.refresh()
...
def complete_task(task_id, completed, tab_panels=None, day_panel=None, task_container=None):
...
def delete_task(task_id, tab_panels=None, day_panel=None, task_container=None):
    """Delete a task"""
    try:
        cursor = db.connection.cursor()
